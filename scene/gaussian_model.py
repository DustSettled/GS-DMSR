...
class GaussianModel:
    ...
    def mask_stable_gaussians(self,optimizer, opacity_history=None,opacity_diff_threshold=0.01, history_window=100):
        
        opacity_history = opacity_history 
        opacity_diff = torch.abs(opacity_history[:,-1] - opacity_history[:,-history_window])
    
        stable_mask = (opacity_diff < opacity_diff_threshold)

        for param_group in optimizer.param_groups:
            if 'opacity' in param_group['name']:
                original_param = param_group['params'][0]
                # print(f"original_param:",original_param.shape)
                stored_state = optimizer.state.get(original_param, None)
                
                
                if original_param.dim() == 2:
                    static_mask = stable_mask.unsqueeze(-1)
                    dynamic_mask = ~static_mask.squeeze()
                else:
                    static_mask = stable_mask
                    dynamic_mask = ~stable_mask

                # 参数分割
                static_params = original_param[static_mask].detach().requires_grad_(False)
                # print(f"static_params:",static_params.shape)
                dynamic_params = original_param[dynamic_mask].requires_grad_(True)
                # print(f"dynamic_params:",dynamic_params.shape)
                new_param = torch.cat([static_params.view(-1, 1), dynamic_params.view(-1, 1)], dim=0)
                
                
                if stored_state is not None:
                    
                    dynamic_mask = ~stable_mask.squeeze()
                    dynamic_exp_avg = stored_state["exp_avg"][dynamic_mask].view(-1, 1)  
                    dynamic_exp_avg_sq = stored_state["exp_avg_sq"][dynamic_mask].view(-1, 1)
                    
                    # 静态部分
                    static_mask = stable_mask.squeeze()
                    static_exp_avg = stored_state["exp_avg"][static_mask].view(-1, 1)    
                    static_exp_avg_sq = stored_state["exp_avg_sq"][static_mask].view(-1, 1)
                    
                    
                    merged_exp_avg = torch.cat([dynamic_exp_avg, static_exp_avg], dim=0)
                    merged_exp_avg_sq = torch.cat([dynamic_exp_avg_sq, static_exp_avg_sq], dim=0)
                    
                    new_state = {
                        "step": stored_state.get("step", 0),
                        "exp_avg": merged_exp_avg,
                        "exp_avg_sq": merged_exp_avg_sq
                    }
