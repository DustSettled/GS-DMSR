...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        with torch.no_grad():
            ...
            opacity_history = torch.empty(0)
            if iteration % 50 == 0 and iteration >= 3000 and iteration <= 4500:
                # print(iteration)
       
                num_gaussians = gaussians._opacity.shape[0]
                # opacity_history = torch.empty(0)
                # num_gaussians = gaussians._opacity.shape[0]
    
                if opacity_history is None or opacity_history.dim() != 2:
                    opacity_history = torch.zeros((num_gaussians, 500), device=gaussians._opacity.device) 
                    
                new_opacity = gaussians._opacity.clone().detach()
                opacity_history = torch.roll(opacity_history, shifts=-1, dims=1)
                opacity_history[:, -1] = new_opacity.squeeze() 
                # print("my word111")
                
                gaussians.mask_stable_gaussians(gaussians.optimizer, opacity_history, opacity_diff_threshold=0.00004, history_window=500)
                # print("my word222")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1000,3000,7000,14000,16000,18000])
